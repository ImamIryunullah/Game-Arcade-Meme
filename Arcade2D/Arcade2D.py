...
start_screen_music = pygame.mixer.Sound('GGGAMING.wav')
game_over_music = pygame.mixer.Sound('illusoryrealm.wav')

background_music = pygame.mixer.music.load('narutoh.wav')

pygame.mixer.init()

screen = pygame.display.set_mode((0,0), FULLSCREEN) #mengatur sizescreen
...
class Background(pygame.sprite.Sprite):
    ...
    # buat method di dalam class background
    def update (self): #memberi value
        self.rect.y += 1
        self.rect.x += 1
...
class Enemy(Player): #subclass
    def __init__(self, img):
        super().__init__(img)
        self.rect.x = random.randrange(80, s_width-80)
        self.rect.x = random.randrange(-500, 0)
...
class Game:
    ...
    def start_text(self):
        ...
        background_image = pygame.image.load('windah.png')

        # Draw background image
        bg_width, bg_height = background_image.get_size()

        # Calculate x, y position to center the image
        x = (s_width - bg_width) // 2
        y = (s_height - bg_height) // 10

        # Draw background image
        screen.blit(background_image, (x, y))
        font = pygame.font.SysFont('Calibri', 50)
        text = font.render('SULTHAN OMBA INAN LAKO', True, 'blue')
        text_rect = text.get_rect(center=(s_width/2, s_height/2 + 50))
        screen.blit(text, text_rect)

        font2 = pygame.font.SysFont('Calibri', 20)
        text2 = font2.render('MADE LAKO', True, 'white')
        text2_rect = text2.get_rect(center=(s_width/2, s_height/2 + 100))
        screen.blit(text2, text2_rect)   
        
        font3 = pygame.font.SysFont('Calibri', 20)
        text_color = 'white'
        text3 = font3.render('Press Enter', True, text_color)
        text3_rect = text3.get_rect(center=(s_width/2, s_height/2 + 130))
    ...
    def game_over_text(self):
        font = pygame.font.SysFont('Calibri', 50)
        text = font.render('Game Over, NT Adik Adik', True, 'red')
        text_rect = text.get_rect(center=(s_width/2, s_height/2))
        screen.blit(text, text_rect)
    ...
    def game_over_screen(self):
        ...
        #instan dari background
    def create_background(self): 
        for i in range(20): #membuat jumlah bintang
            x = random.randint(1,6) #mengatur size bintang
    ...
    def playerbullet_hits_enemy(self):
        hits = pygame.sprite.groupcollide(enemy_group, playerbullet_group, False
            , True)
        for i in hits:
            self.count_hit += 1
            if self.count_hit == 3:
    ...
    def playerbullet_hits_ufo(self):
        hits = pygame.sprite.groupcollide(ufo_group, playerbullet_group, False
            , True)
        for i in hits:
            self.count_hit2 += 1
    ...
    def create_score(self):
        ...
        text = font.render("Score Antum : " + str(score), True, 'green')
